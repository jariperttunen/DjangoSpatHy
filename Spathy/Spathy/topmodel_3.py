...
******************************************************************************
spathy.modules.topmodel

TopModel (Beven & Kirkby) -implementations for SpatHy -integration
...
class Topmodel_Homogenous():
    def __init__(self, pp, cellarea, cmask, flowacc, slope, S_initial=None,
        ...
        """ 
        Compute local xi and catchment average X hydrologic similarity indices. Set highest 2.5% of
        distribution equal to the threshold value; this concerns mainly the stream network cells.
        """
        slope_rad = np.radians(self.slope)  # deg to rad
        xi = np.log(self.a / cellsize / (np.tan(slope_rad) + eps))
        # set highest 2.5 % of twi into largest class
        print('2',cmask)
        clim = np.percentile(xi[xi>0], 97.5)
        xi[xi>clim] = clim
        self.xi = xi
        # self.xi[self.xi > 20.0] = 20.0  # set limits for xi
        # self.xi = np.log(self.a / (rad + eps))        
        self.X = 1.0 / self.CatchmentArea*np.nansum(self.xi*self.CellArea)
        # print('X', self.X)
...
#        roff[np.where(s <= 0)] = R
        ...
        # average returnflow per unit area
        Qf = np.nansum(qr)*self.CellArea / self.CatchmentArea
