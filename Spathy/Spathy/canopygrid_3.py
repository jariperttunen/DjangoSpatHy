...
class CanopyGrid():
    def __init__(self, cpara, lai_conif=None, lai_decid=None, cf=None, hc=None,  cmask=None, outputs=False):
        """
    ...
    def dry_canopy_et(self, D, Qp, AE, Ta, Ra=25.0, Ras=250.0, CO2=380.0, f=1.0, Rew=1.0, beta=1.0, fPheno=1.0):
        ...
        Efloor = penman_monteith(tau * f * AE, 1e3*D, Ta, gcs, 1./Ras, units='mm')
...
def test_canopygrid():
    # script for testing
    from iotools import read_HydeDaily, create_catchment
    ffile = 'c:\pyspace\spathy\ini\spathy_default.ini'
    
    _, FORC = read_HydeDaily('c:\pyspace\spathy\data\HydeDaily2000-2010.txt')
    
    pgen, cpara = read_ini(ffile)
    print(cpara)
    gisdata = create_catchment(pgen['catchment_id'], fpath=pgen['gis_folder'],
                               plotgrids=False, plotdistr=False)
                               
    LAIc = gisdata['LAI_pine'] + gisdata['LAI_spruce']
    LAId = gisdata['LAI_decid']
    # LAId = None
    cf = gisdata['cf']
    ba = gisdata['ba']
    #cf = 0.1939 * ba / (0.1939 * ba + 1.69) + eps
    hc = gisdata['hc']
    cpy = CanopyGrid(cpara, lai_conif=LAIc, lai_decid=LAId, cf=cf, hc=hc, outputs=True)  
    ...
    for k in range(fstep, fstep + Nsteps):
        # print 'k=' + str(k)
        # forcing
        doy = FORC['doy'].iloc[k]; ta = FORC['Ta'].iloc[k]
        vpd = FORC['VPD'].iloc[k]; rg = FORC['Rg'].iloc[k]
        par = FORC['Par'].iloc[k]; prec = FORC['Prec'].iloc[k]
        
        # run CanopyGrid
        potinf, trfall, interc, evap,et, mbe = cpy.run_timestep(doy, dt, ta, prec, rg, par, vpd, Rew=1.0, P=101300.0)
        # print('k:', np.nanmax(cpy._LAIdecid / cpy._LAIdecid_max), cpy.results['LAIfract'][k])
    return cpy, FORC, gisdata                              
