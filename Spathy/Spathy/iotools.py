...
#spathypath.spathy_path = os.path.join('./')
#os.chdir(spathypath.spathy_path)
...
def read_setup(inifile):
    ...
    """
    print('iotools read_setup inifile:',inifile)
...
def create_catchment(ID, fpath=spathypath.SVEcatchmentpath, plotgrids=False, plotdistr=False):
    """
    ...
    print('create _catchment:',fpath)
    bname = pathlib.Path(str(ID),'sve_'+str(ID)+'_')
    fpath = pathlib.Path(fpath,bname)
    ...
    dem, info, pos, cellsize, nodata = read_AsciiGrid(str(pathlib.Path(str(fpath)+'dem_16m_aggr.asc')))
    ...
    flowacc, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'Flow_accum_D-Inf_grids.asc')))
    flowacc = flowacc*cellsize**2  # in m2
    # slope, degrees
    slope, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'slope_16m.asc')))
    # twi
    twi, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath)+ 'TWI_16m.asc')))

    # Maastotietokanta water bodies: 1=waterbody
    stream, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath)+'vesielementit_mtk.asc')))
    stream[np.isfinite(stream)] = 1.0
    # maastotietokanta peatlandmask
    peatm, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath)+ 'suo_mtk.asc')))
    peatm[np.isfinite(peatm)] = 1.0
    # maastotietokanta kalliomaski
    rockm, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath)+ 'kallioalue_mtk.asc')))
    rockm[np.isfinite(rockm)] = 1.0

    """ stand data (MNFI)"""
    # stand volume [m3ha-1]
    vol, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath)+'tilavuus.asc')), setnans=False)
    vol = vol*cmask
    ...
    # basal area [m2 ha-1]
    ba, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'ppa.asc')))
    ba[ix_n] = nofor['ba']
    ba[ix_p] = opeatl['ba']
    ba[ix_w] = np.NaN

    # tree height [m]
    height, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'keskipituus.asc')))
    height = 0.1*height  # m
    ...
    # canopy closure [-]    
    cf, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'latvuspeitto.asc')))
    cf = 1e-2*cf
    cf[ix_n] = nofor['cf']
    cf[ix_p] = opeatl['cf']
    cf[ix_w] = np.NaN
    ...
    # stand age [yrs]
    age, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath)+'ika.asc')))
    age[ix_n] = nofor['age']
    age[ix_p] = opeatl['age']
    age[ix_w] = np.NaN

    # leaf biomasses and one-sided LAI
    bmleaf_pine, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'bm_manty_neulaset.asc')))
    bmleaf_spruce, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'bm_kuusi_neulaset.asc')))
    bmleaf_decid, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'bm_lehtip_neulaset.asc')))
    ...
    bmroot_pine, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'bm_manty_juuret.asc')))
    bmroot_spruce, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'bm_kuusi_juuret.asc')))
    bmroot_decid, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath)  + 'bm_lehtip_juuret.asc')))  
    ...
    # site types
    maintype, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'paatyyppi.asc')))
    maintype = maintype*cmask
    sitetype, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'kasvupaikka.asc')))
    ...
    gtk_s, _, _, _, _ = read_AsciiGrid(str(pathlib.Path(str(fpath) + 'soil.asc'))) 
...
def read_AsciiGrid(fname, setnans=True):
    ...
    """
    import numpy as np
    print('readAsciiGrid fname:',fname)
    fid = open(fname, 'r')
...
def read_FMI_weather(ID, start_date, end_date, sourcefile='data/SVEcatchments/FMI/SVE_saa.csv', CO2=380.0):
    ...
    sourcefile = pathlib.Path(spathypath.spathy_path, sourcefile)
    ID = int(ID)
    sourcefile=str(sourcefile)
    print('read_FMI_weather sourcefile:',sourcefile)
    # import forcing data
    ...
    #SettingWithCopyWarning  suggest to rewrite like this
    fmi.loc[np.isnan(fmi['Prec']),'Prec'] = 0.0
...
#'data/SVEcatchments/Runoff/Runoffs_SVEcatchments_mmd.csv'
def read_SVE_runoff(ID, start_date,end_date, sourcefile=spathypath.SVErunoff_file):
    """
    reads FMI interpolated daily weather data from file
    IN:
    ...
    sourcefile = pathlib.Path(spathypath.spathy_path, sourcefile)
    sourcefile = str(sourcefile)
    print('read_SVE_runoff sourcefile',sourcefile)
